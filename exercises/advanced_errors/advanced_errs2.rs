...
impl FromStr for Climate {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let (city, year, temp) = match &v[..] {
            arr if arr.len() > 3 => return Err(ParseClimateError::BadLen),
            arr if arr.len() > 1 && arr.len() < 3 => return Err(ParseClimateError::BadLen),
            [city, year, temp] if city.len() == 0 => return Err(ParseClimateError::NoCity),
            [city, year, temp] => (city.to_string(), year, temp),
            _ => return Err(ParseClimateError::Empty),
        };
        let year: u32 = year.parse()?; // ParseIntError
        let temp: f32 = temp.parse()?; // ParseFloatError
